...
def simple_request(func_name, query, variables):
    request = requests.post('https://github.com', json={'query': query, 'variables': variables}, headers=HEADERS)
...
def recursive_loc(owner, repo_name, data, cache_comment, addition_total=0, deletion_total=0, my_commits=0, cursor=None):
    ...
    if request.status_code == 403:
        raise Exception('Hit GraphQL anti-abuse limits!')
    raise Exception('recursive_loc failed', request.status_code, request.text)

def loc_counter_one_repo(owner, repo_name, data, cache_comment, history, addition_total, deletion_total, my_commits):
    for node in history['edges']:
        if node['node'] and node['node']['author'] and node['node']['author']['user']:
...
def loc_query(owner_affiliation, comment_size=0, force_cache=False, cursor=None, edges=None):
    ...
    res_data = request.json()['data']['user']['repositories']
    
    if res_data['pageInfo']['hasNextPage']:
        edges += res_data['edges']
        return loc_query(owner_affiliation, comment_size, force_cache, res_data['pageInfo']['endCursor'], edges)
    return cache_builder(edges + res_data['edges'], comment_size, force_cache)
...
def cache_builder(edges, comment_size, force_cache, loc_add=0, loc_del=0):
    ...
    for index in range(len(edges)):
        if not edges[index] or not edges[index]['node'] or not edges[index]['node']['defaultBranchRef']: continue
...
def add_archive():
    try:
        with open('cache/repository_archive.txt', 'r') as f: data = f.readlines()
        old_data = data
...
def follower_getter(username):
    ...
    return simple_request(follower_getter.__name__, query, {'login': username}).json()['data']['user']['followers']['totalCount']
...
if __name__ == '__main__':
    ...
    raw_commits = f"{commit_data:,}"
    raw_stars = f"{star_data:,}"
    raw_repos = f"{repo_data:,}"
    raw_contrib = f"{contrib_data:,}"
    raw_followers = f"{follower_data:,}"
